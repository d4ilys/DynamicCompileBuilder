...
namespace DynamicBuilder
{
    internal class Builder
    {
        ...
        /// <returns></returns>
        public Builder UseClass(string className, TableAttribute tableAttribute)
        {
        ...
        /// <returns></returns>
        public Builder UseProperty(string propertyName, Type propertyType, ColumnAttribute columnAttribute)
        {
        ...
        public Type Build()
        {
            ...
            var assemblyName = new AssemblyName("Daily.Dynamics");
            //设置程序集的名称
            var defineDynamicAssembly = AssemblyBuilder.DefineDynamicAssembly(assemblyName, AssemblyBuilderAccess.Run);
            //动态在程序集内创建一个模块
            var defineDynamicModule = defineDynamicAssembly.DefineDynamicModule("FreeSql.Daily.Dynamics");
            //动态的在模块内创建一个类
        ...
        public static object? CreateObjectByType(Type type, Dictionary<string, object> porpertys)
        {
            ...
            if (istance == null)
                return null;
            //根据字典中的key确定缓存
            var cacheKey = string.Join("-", porpertys.Keys.Order());
            var dynamicDelegate = _delegateCache.GetOrAdd(cacheKey, key =>
            {
                ...
                Console.WriteLine(Expression.Lambda(block, typeParam, dicParam).ToString("C#"));
